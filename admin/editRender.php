...
if(isset($_POST['submit'])) {
    if (isset($_POST['edit'])) {
        ...
        if ($conn->query($query) === TRUE) {
            ...
            header("location: index.php?menu=adminRenders");
        } else {
...
#DELETE RENDER
    ...
    if (mysqli_num_rows($newsInfoQuery) != 0) {
        ...
        if (mysqli_num_rows($imageInfoQuery) != 0) {
            ...
            print $imageInfo['id'];
            $deleteImageQuery = mysqli_query($conn, "DELETE FROM images WHERE id = " . $imageInfo['id'] . " ;");
    ...
    print '
            ...
            <h1>Create render</h1>
                <form method="post" action="" enctype="multipart/form-data">
                    ...
                    <input type="hidden" id="archived" name="archived" value="N">';
    if ($_SESSION['user']['role'] == "ADMIN" || $_SESSION['user']['role'] == "EDITOR") {
        print '<input type="hidden" id="approved" name="approved" value="Y">';
    } else {
        print '<input type="hidden" id="approved" name="approved" value="N">';
    }
    print '
